<?php
// Fetch filters directly since auth is already handled by index.php
...
<div class="flex items-center gap-4 mb-6 border-b border-slate-300 pb-4">
    ...
    <?php foreach($allowed_filters as $f): ?>
        <a href="?page=applications&filter=<?= $f ?>" class="px-4 py-1 text-xs font-bold uppercase tracking-widest <?= $filter === $f ? 'bg-cyan-600 text-white' : 'bg-white border border-slate-300 text-slate-600 hover:bg-slate-200' ?> transition-colors">
            <?= $f ?>
        </a>
    <?php endforeach; ?>
</div>
